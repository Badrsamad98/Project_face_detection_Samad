...
flags.DEFINE_string('img_path', './data/AFFLECK_BEN2.jpg',
                    'path to input image')
...
def main(_argv):
    ...
    if not FLAGS.webcam:
        ...
        save_img_path = os.path.join(os.path.split(FLAGS.img_path)[
                                     0], 'out_' + os.path.split(FLAGS.img_path)[1])
        ...
        image = process_func(FLAGS.img_path, X[0])
        path = os.path.join(os.path.split(FLAGS.img_path)[
                            0], 'crop_' + os.path.split(FLAGS.img_path)[1])
